...
//  UPDATE  FETCH HISTORIES  
      ...
      if (brtype & IS_BR_INDIRECT)
	{
      ...
      HistoryUpdate (pc, brtype, taken, target, Fetch_ptghist,
			...
			folded_history * G, folded_history * J)
    {
      ...
      if (!(brtype & IS_BR_INDIRECT))     if (brtype & IS_BR_CALL)
	maxt = 5;
...
//  UPDATE  RETIRE HISTORY PATH  
      HistoryUpdate (pc, brtype, taken, target,  Retire_ptghist,
